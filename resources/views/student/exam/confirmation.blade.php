                ...
                <div class="card-body p-5">
                    <div class="row align-items-center">
                        <div class="col-md-5 text-center mb-4 mb-md-0">
                            ...
                            <h4 class="font-weight-bold text-gray-800">{{ $session->subject->name ?? 'Mata Pelajaran' }}
                            </h4>
                            <p class="text-gray-500 mb-0">
                                {{ $session->examPackage->title ?? 'Paket Soal Acak / Semua Soal' }}
                            </p>
                        </div>
                        ...
                        <div class="col-md-7">
                            <table class="table table-borderless mb-0">
                                <tr>
                                    ...
                                    <td class="font-weight-bold text-gray-800">@tgl_jam($session->start_time)</td>
                                </tr>
                                <tr>
                                    <th class="text-gray-500 pl-0">Berakhir</th>
                                    <td class="font-weight-bold text-gray-800">@tgl_jam($session->end_time)</td>
                                </tr>
                                <tr>
                    ...
                    <div class="alert bg-yellow-50 border-yellow-200 text-yellow-800 rounded-lg p-4 mt-4">
                        <div class="d-flex">
                            ...
                            <div class="ml-3">
                                ...
                                <ul class="text-sm list-disc pl-4 space-y-1 mb-0">
                                    <li>Pastikan koneksi internet stabil.</li>
                                    <li>Waktu akan berjalan mundur otomatis setelah menekan tombol mulai.</li>
                                    <li>Jangan menutup browser atau berpindah aplikasi selama ujian berlangsung.</li>
                                    <li>Stimulus, gambar, dan soal akan tampil pada halaman pengerjaan.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    @if(!($attempt && $attempt->status == 'in_progress'))
                        <div class="mt-4">
                            <div class="form-group text-center">
                                ...
                                <input type="text" id="token" name="token" form="start-form"
                                    class="form-control text-center text-uppercase font-weight-bold mx-auto border-2 border-primary"
                                    style="max-width: 220px; font-size: 1.5rem; letter-spacing: 5px;" placeholder="_____"
                                    maxlength="10" required autocomplete="off">
                                <small class="text-muted d-block mt-2">Minta token kepada pengawas ujian.</small>
                            </div>
                        </div>
                    @else
                        <div class="alert alert-info rounded-lg p-4 mt-4 mb-0">
                            <i class="fas fa-info-circle mr-2"></i>
                            Anda memiliki ujian yang sedang berlangsung. Klik tombol lanjutkan untuk kembali mengerjakan.
                        </div>
                    @endif

                    <div class="mt-5 text-center">
                        <form id="start-form"
                            action="{{ request()->route('subdomain') ? route('institution.student.exam.start', ['subdomain' => request()->route('subdomain'), 'id' => $session->id]) : route('student.exam.start', $session->id) }}"
                            method="POST">
                            @csrf
                            ...
                            @if($attempt && $attempt->status == 'in_progress')
                                <button type="submit"
                                    class="btn btn-warning py-3 px-8 rounded-xl font-weight-bold text-white shadow-lg transform hover:scale-105 transition">
                                    <i class="fas fa-play mr-2"></i> Lanjutkan Ujian
                                </button>
                            @else
                                <button type="submit"
                                    class="btn btn-primary py-3 px-8 rounded-xl font-weight-bold shadow-lg transform hover:scale-105 transition"
