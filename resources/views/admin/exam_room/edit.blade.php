    ...
    <div class="col-md-6">
        <div class="card shadow-sm border-0 rounded-lg">
            <div class="card-header bg-white">
                <h5 class="mb-0 font-weight-bold">Form Edit Ruangan</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.exam_room.update', $room->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="form-group mb-4">
                        <label for="name" class="font-weight-bold text-gray-700">Nama Ruangan <span class="text-danger">*</span></label>
                        <input type="text" name="name" id="name" class="form-control rounded-lg @error('name') is-invalid @enderror" value="{{ old('name', $room->name) }}" placeholder="Contoh: Lab Komputer 1" required>
                        @error('name')
